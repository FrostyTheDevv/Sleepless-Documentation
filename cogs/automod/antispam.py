...
class AntiSpam(commands.Cog):
    def __init__(self, bot):
        ...
        # Removed in-memory storage: self.recent_messages = {}

    async def save_recent_message(self, user_id, guild_id, timestamp):
        """Save a recent message timestamp to the database"""
        async with aiosqlite.connect('db/antispam.db') as db:
            await db.execute('''
                INSERT INTO recent_messages (user_id, guild_id, timestamp)
                VALUES (?, ?, ?)
            ''', (user_id, guild_id, timestamp))
            await db.commit()

    async def get_recent_messages(self, user_id, guild_id, time_window=10):
        """Get recent messages for a user within the time window"""
        cutoff_time = time.time() - time_window
        async with aiosqlite.connect('db/antispam.db') as db:
            cursor = await db.execute('''
                SELECT timestamp FROM recent_messages
    ...
    async def cleanup_old_messages(self):
        ...
        async with aiosqlite.connect('db/antispam.db') as db:
            await db.execute('DELETE FROM recent_messages WHERE timestamp < ?', (cutoff_time,))
            await db.commit()
    ...
    async def on_message(self, message):
        ...
        # Save the current message to database
        await self.save_recent_message(user.id, guild_id, current_time)

        # Get recent messages and filter to 10-second window
        user_messages = await self.get_recent_messages(user.id, guild_id, 10)

        # Clean up old messages periodically (every 100 messages or so)
        if len(user_messages) > 10:  # If we have many messages, clean up
        ...
        if len(user_messages) > self.spam_threshold:
            ...
            try:
                ...
                simple_embed.description = f"<:feast_tick:1400143469892210753> | {user.mention} has been successfully **{action_taken}** for **Spamming.**"
