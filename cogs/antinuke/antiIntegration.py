...
class AntiIntegration(commands.Cog):
    ...
    async def on_guild_integrations_update(self, guild):
        ...
        log_entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.integration_create)
        if not log_entry:
            return
        ...
        if await is_whitelisted(guild.id, executor_member, 'serverup'):
            return
        
        await self.tracker.track_action(
            guild_id=guild.id,
            user_id=executor.id,
            action_type="integration_create",
            metadata={
                "integration_name": log_entry.target.name if hasattr(log_entry.target, 'name') else "Unknown",
        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} integration creates in {config.time_window}s (Level {escalation_level})"
                )
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=0,
                    reason=f"Created {action_count} integrations in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiIntegration: {e}")
