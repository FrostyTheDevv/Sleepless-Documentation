...
class AntiPrune(commands.Cog):
    ...
    async def on_member_remove(self, member):
        ...
        if await is_whitelisted(guild.id, executor_member, 'prune'):
            return
        ...
        await self.tracker.track_action(
            ...
            metadata={
                "members_pruned": log_entry.extra.members_removed if hasattr(log_entry.extra, 'members_removed') else 1,
        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} member prunes in {config.time_window}s (Level {escalation_level})"
                )
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=0,
                    reason=f"Pruned members {action_count} times in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiprune: {e}")
