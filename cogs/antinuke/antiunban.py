        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} unbans in {config.time_window}s (Level {escalation_level})"
                )
                
                # Re-ban the unbanned user
                try:
                    await guild.ban(user, reason=f"Re-banning user unbanned by {executor} (threshold breach)")
                except:
                    pass
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=1,
                    reason=f"Unbanned {action_count} users in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiunban: {e}")
