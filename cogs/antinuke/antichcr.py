...
class AntiChannelCreate(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel):
        ...
        audit_entries = await self.fetch_audit_logs(guild, discord.AuditLogAction.channel_create)
        ...
        if await is_whitelisted(guild.id, executor_member, 'chcr'):
            return
        ...
        if should_punish:
            await self.apply_punishment(guild, executor, channel, action_count, config, reason)

    async def apply_punishment(self, guild, executor, last_created_channel, action_count, config, trigger_reason=""):
        try:
            ...
            base_reason = trigger_reason if trigger_reason else f"{action_count} channel_creates in {config.time_window}s"
            ...
            for action in recent_actions:
                ...
                try:
                    ...
                    if target_channel:
                        await target_channel.delete(reason=f"Channel created by {executor} (threshold breach)")
                        deleted_count += 1
            ...
            await self.tracker.log_punishment(
                ...
                reason=f"Created {action_count} channels in {config.time_window} seconds",
                escalation_level=escalation_level
            )
            ...
            print(f"Error applying punishment in antichcr: {e}")
