...
class AntiRoleUpdate(commands.Cog):
    ...
    async def on_guild_role_update(self, before, after):
        ...
        if await is_whitelisted(guild.id, executor_member, 'rlup'):
            return
        ...
        await self.tracker.track_action(
            ...
            metadata={
                ...
                "old_permissions": before.permissions.value,
                "new_permissions": after.permissions.value,
                "old_color": str(before.color),
                "new_color": str(after.color),
    ...
    async def apply_punishment(self, guild, executor, before, after, action_count, config):
        try:
            ...
            punishment_applied = await apply_escalated_punishment(
                ...
                reason=f"Antinuke: {action_count} role_updates in {config.time_window}s (Level {escalation_level})"
                ...
                try:
                    ...
                    if target_role:
                        await target_role.edit(
                            name=action.metadata.get("old_name", target_role.name),
                            permissions=discord.Permissions(action.metadata.get("old_permissions", 0)),
                            color=discord.Color(int(action.metadata.get("old_color", "#000000").replace("#", ""), 16)),
                            reason=f"Reverting changes by {executor} (threshold breach)"
                        )
                        reverted_count += 1
                        await asyncio.sleep(0.5)
                except Exception as e:
                    print(f"Failed to revert role update: {e}")
            ...
            await self.tracker.log_punishment(
                ...
                reason=f"Updated {action_count} roles in {config.time_window} seconds"
            ,
                escalation_level=escalation_level
            )
            
        except Exception as e:
            print(f"Error applying punishment in antirlup: {e}")
