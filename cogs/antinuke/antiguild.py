...
class AntiGuild(commands.Cog):
    ...
    async def on_guild_update(self, before, after):
        ...
        log_entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.guild_update)
        if not log_entry:
            return
        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} guild updates in {config.time_window}s (Level {escalation_level})"
                )
                
                # Revert guild name if changed
                if before.name != after.name:
                    try:
                        await guild.edit(name=before.name, reason="Reverting unauthorized guild name change")
                    except:
                        pass
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=1,
                    reason=f"Updated guild {action_count} times in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiguild: {e}")
