...
class AntiEmojiDelete(commands.Cog):
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        ...
        log_entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.emoji_delete)
        if not log_entry:
            return
        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} emoji deletes in {config.time_window}s (Level {escalation_level})"
                )
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=0,
                    reason=f"Deleted {action_count} emojis in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiemodl: {e}")
