...
class AntiEmojiUpdate(commands.Cog):
    ...
    async def on_guild_emojis_update(self, guild, before, after):
        if len(after) != len(before):
            return  # Only track updates, not creates/deletes
        ...
        log_entry = await self.fetch_audit_logs(guild, discord.AuditLogAction.emoji_update)
        if not log_entry:
            return
        ...
        await self.tracker.track_action(
            ...
            metadata={
                "emoji_name": log_entry.target.name if hasattr(log_entry.target, 'name') else "Unknown",
        ...
        if exceeded:
            try:
                ...
                punishment_applied = await apply_escalated_punishment(
                    ...
                    duration=duration,
                    reason=f"Antinuke: {action_count} emoji updates in {config.time_window}s (Level {escalation_level})"
                )
                ...
                await self.tracker.log_punishment(
                    ...
                    actions_reverted=0,
                    reason=f"Updated {action_count} emojis in {config.time_window} seconds",
                    escalation_level=escalation_level
                )
            except Exception as e:
                print(f"Error applying punishment in antiemoup: {e}")
