...
class Vanityroles(commands.Cog):
            ...
            if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
                if role.permissions.administrator or role.permissions.ban_members or role.permissions.kick_members:
                    embed1 = discord.Embed(
                        description=
                        "<:feast_dot:1400140945617588264> | Please Select Role That Dont Have Any Type Of Dangerous Permissions.",
                        color=0x00FFCA)
                    ...
                    embed.add_field(
                        name="<:feast_dot:1400140945617588264> Vanity",
                        value=f"{vanity}",
                        inline=False)
                    embed.add_field(
                        name="<:feast_dot:1400140945617588264> Role",
                        value=f"{role.mention}",
                        inline=False)
                    embed.add_field(
                        name="<:feast_dot:1400140945617588264> Channel",
    ...
    async def config(self, ctx):
        ...
        if str(ctx.guild.id) not in jnl:
            embed1 = discord.Embed(
                description=
                "<:feast_dot:1400140945617588264> | This server don't have any vanity roles setupped yet .",
                color=0x00FFCA)
            await ctx.reply(embed=embed1, mention_author=False)
            ...
            embed.add_field(name="<:feast_dot:1400140945617588264> Vanity",
                            value=f"{vanity}",
                            inline=False)
            embed.add_field(name="<:feast_dot:1400140945617588264> Role",
                            value=f"{randirole.mention}",
                            inline=False)
            embed.add_field(name="<:feast_dot:1400140945617588264> Channel",
