...
class Ship(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.special_users = [1385303636766359612,]

    @commands.hybrid_command(pass_context=True, help="Ship two users together.")
    @blacklist_check()
    @ignore_check()
    async def ship(self, ctx, user1: typing.Optional[discord.Member] = None, user2: typing.Optional[discord.Member] = None):
        ...
        # Safe attribute access and None checks
        if not user1 or not hasattr(user1, 'id') or not user2 or not hasattr(user2, 'id'):
            await ctx.send("Could not determine users to ship.")
            return

        author_id = float(getattr(user1, 'id', 0))
        user_id = float(getattr(user2, 'id', 0))

        if getattr(user1, 'id', None) in self.special_users and getattr(user2, 'id', None) in self.special_users:
            rate = 100
        else:
        ...
        user_avatar = await get_avatar(user2) if user2 else None
        author_avatar = await get_avatar(user1) if user1 else None

        user1_name = getattr(user1, 'name', 'Unknown')
        user2_name = getattr(user2, 'name', 'Unknown')
        user1_mention = getattr(user1, 'mention', user1_name)
        user2_mention = getattr(user2, 'mention', user2_name)

        if user_avatar and author_avatar:
            self.make_image(author_avatar, user_avatar, user1_name, user2_name, rate)
            await self.img_ship(ctx, user1_mention, user2_mention, rate)
        else:
            await self.text_ship(ctx, user1_mention, user2_mention, rate)
