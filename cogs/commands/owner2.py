    ...
    async def global_ban(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Bans [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
            ...
            async def list_failure(interaction):
                ...
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Bans [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_kick(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Kicks [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Kicks [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_timeout(self, ctx: commands.Context, user: discord.User, reason: str = "Severe violations of Discord's terms of service."):
        ...
        async def confirm(interaction):
            ...
            for guild in mutual_guilds:
                ...
                if member:
                    try:
                        ...
                        timeout_end = self.tz_helpers.get_utc_now() + timedelta(days=28)
                        await member.edit(timed_out_until=timeout_end, reason=reason)
                        success.append(guild.name)
                    except Exception as e:
                        print(f"Failed to timeout {user} in {guild.name}: {e}")
                        failure.append(guild.name)
            embed = discord.Embed(
                title="Success",
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Timeouts [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Timeouts [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_nick(self, ctx: commands.Context, user: discord.User, *, name: str):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Nickname Change [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Nickname Change [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_clearnick(self, ctx: commands.Context, user: discord.User):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Nickname Clear [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Nickname Clear [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
    ...
    async def global_freezenick(self, ctx: commands.Context, user: discord.User, *, name: str):
        ...
        async def confirm(interaction):
            ...
            async def list_success(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(success)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Successful Freezes [{len(success)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
            ...
            async def list_failure(interaction):
                ...
                entries = [f"{i+1}. {name}" for i, name in enumerate(failure)]
                paginator = Paginator(
                    source=DescriptionEmbedPaginator(entries=entries, description="", title=f"Unsuccessful Freezes [{len(failure)}]", color=0x006fb9, per_page=10),
                    ctx=ctx
                )
                await paginator.paginate()
