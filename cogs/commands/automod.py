...
class Automod(commands.Cog):
    ...
    async def enable(self, ctx):
        ...
        if ctx.author != ctx.guild.owner and ctx.author.top_role.position < ctx.guild.me.top_role.position:
            embed = discord.Embed(title="<:feast_cross:1400143488695144609> Access Denied", description="Your top role must be at the **same** position or **higher** than my top role.", color=0x006fb9)
        ...
        embed = discord.Embed(title=f"{ctx.guild.name}'s Automod Setup", color=0x006fb9)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.description = "\n".join([f"<:feast_cross:1400143488695144609><:feast_tick:1400143469892210753> : {event}" for event in events])
    ...
    async def enable_automod(self, ctx, guild_id, selected_events, interaction):
        ...
        embed = discord.Embed(title="Automod Enabled Successfully", color=0x006fb9)
        embed.description = "\n".join([f"<:feast_cross:1400143488695144609><:feast_tick:1400143469892210753> : {event}" for event in selected_events] +
                                       [f"<:feast_cross:1400143488695144609><:feast_tick:1400143469892210753> : {event}" for event in ["Anti spam", "Anti caps", "Anti link", "Anti invites", "Anti mass mention", "Anti emoji spam", "Anti NSFW link"] if event not in selected_events])
        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "Sleepless-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"Sleepless-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("Sleepless-automod", overwrites=overwrites)
    ...
    async def punishment(self, ctx):
        ...
        embed = discord.Embed(title=f"Current Automod Punishments for {ctx.guild.name}", color=0x006fb9)
        punishment_map = {}
        ...
        async def select_callback(interaction):
            ...
            for punishment in ["Mute", "Kick", "Ban"]:
                ...
                async def punishment_callback(interaction, selected_events=selected_events, punishment=punishment):
                    if interaction.user != ctx.author:
                    ...
                    updated_embed = discord.Embed(title=f"Updated Automod Punishments for {ctx.guild.name}", color=0x006fb9)
    ...
    async def ignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            success = discord.Embed(title="<:feast_tick:1400143469892210753> Channel Whitelisted", description=f"The channel {channel.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x006fb9)
    ...
    async def ignore_role(self, ctx, role: discord.Role):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            if await cursor.fetchone() is not None:
                embed = discord.Embed(title="__Role Already Whitelisted!__", description=f"<:feast_cross:1400143488695144609> The role {role.mention} is already in the ignore list.\n\n➜ Use **{ctx.prefix}automod unignore role {role.mention}** to remove it.", color=0x006fb9)
                embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
                await ctx.send(embed=embed)
                return
            ...
            success = discord.Embed(title="<:feast_tick:1400143469892210753> Role Whitelisted", description=f"The role {role.mention} has been added to the ignore list \n\n➜ Use `{ctx.prefix}automod ignore show` to view the ignore list.", color=0x006fb9)
            success.set_thumbnail(url=self.bot.user.avatar.url)
            success.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
    ...
    async def ignore_show(self, ctx):
        ...
        embed = discord.Embed(title="Ignored Channels & Roles for Automod", color=0x006fb9)
    ...
    async def ignore_reset(self, ctx):
        ...
        async with aiosqlite.connect("db/automod.db") as db:
            ...
            await db.commit()
        embed=discord.Embed(title=f"Automod Settings for {ctx.guild.name}", description=f"** <:feast_tick:1400143469892210753> | All ignored channels and roles have been reset!**\n\nTo view current Automod settings use `{ctx.prefix}automod config`", color=0x006fb9)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
        embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
               icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=embed)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:feast_tick:1400143469892210753> Success", description=f"{channel.mention} has been removed from the automod ignore list.", color=0x006fb9)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:feast_cross:1400143488695144609> Error", description=f"{channel.mention} is not in the automod ignore list.", color=0x006fb9)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            embed = discord.Embed(title="<:feast_tick:1400143469892210753> Success", description=f"{role.mention} has been removed from the automod ignore list.", color=0x006fb9)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title="<:feast_cross:1400143488695144609> Error", description=f"{role.mention} is not in the automod ignore list.", color=0x006fb9)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def disable(self, ctx):
        ...
        if view.value is None:
            ...
            embed.title = "<:feast_tick:1400143469892210753> Automod Disabled"
    ...
    async def config(self, ctx):
        ...
        current_punishments = await self.get_current_punishments(guild_id)
        embed = discord.Embed(title=f"Enabled Automod Events & their punishment type for {ctx.guild.name}", color=0x006fb9)
    ...
    async def logging(self, ctx):
        ...
        if not await self.is_automod_enabled(guild_id):
            ...
            await ctx.send(embed=embed)
            return
        # Get current logging channel
        async with aiosqlite.connect("db/automod.db") as db:
            cursor = await db.execute("SELECT log_channel FROM automod_logging WHERE guild_id = ?", (guild_id,))
            current_log_channel_id = await cursor.fetchone()
            current_log_channel = ctx.guild.get_channel(current_log_channel_id[0]) if current_log_channel_id and current_log_channel_id[0] else None
            
        # Create paginated channel selector for logging channel
        async def automod_logging_callback(interaction, select):
            if interaction.user != ctx.author:
                await interaction.response.send_message("You are not authorized to set the automod logging channel.", ephemeral=True)
                return
            ...
            async with aiosqlite.connect("db/automod.db") as db:
                await db.execute("INSERT OR REPLACE INTO automod_logging (guild_id, log_channel) VALUES (?, ?)", (guild_id, selected_channel_id))
                await db.commit()

            embed = discord.Embed(
                title=f"Automod Settings for {ctx.guild.name}",
                description=f"**<:feast_tick:1400143469892210753> | Automoderation Logging channel set to {selected_channel.mention}.**\n\n➜ Use `{ctx.prefix}automod config` to view current Automod settings.",
                color=0x006fb9
            )
            embed.set_footer(text=f'"{ctx.command.qualified_name}" Command executed by {ctx.author}',
                           icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        ...
        view = PaginatedChannelView(
            ...
            exclude_channels=[],
            custom_callback=automod_logging_callback,
            timeout=300
        )
        ...
        embed = discord.Embed(
            title=f"Automod Logging Channel for {ctx.guild.name}",
            description=f"Current Logging Channel: {current_log_channel.mention if current_log_channel else 'None'}",
            color=0x006fb9
        )
        
        footer_text = "Use the dropdown menu to select a logging channel."
