def to_user(obj):
    # If obj is a Member, return the underlying User, else return obj
    return getattr(obj, '_user', obj)
import discord
import typing
...
class Games(Cog):
    ...
    async def _chess(self, ctx: Context, player: discord.Member):
        if player == ctx.author:
            ...
            game = btn.BetaChess(white=to_user(ctx.author), black=to_user(player))
    ...
    @commands.max_concurrency(5, per=commands.BucketType.default, wait=False)
    @commands.guild_only()
    async def _rps(self, ctx: Context, player: typing.Optional[discord.Member] = None):
        other_player = to_user(player) if player is not None else None
        game = btn.BetaRockPaperScissors(other_player)
    ...
    async def _ttt(self, ctx: Context, player: discord.Member):
        if player == ctx.author:
            ...
            game = btn.BetaTictactoe(cross=to_user(ctx.author), circle=to_user(player))
    ...
    async def _battle(self, ctx: Context, player: discord.Member):
        game = btn.BetaBattleShip(player1=to_user(ctx.author), player2=to_user(player))
        await game.start(ctx)
    ...
    async def _connectfour(self, ctx: Context, player: discord.Member):
        if player == ctx.author:
            ...
            game = games.ConnectFour(red=to_user(ctx.author), blue=to_user(player))
