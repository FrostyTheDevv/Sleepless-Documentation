...
class MapView(ui.View):
    def __init__(self, bot, location, ctx):
        ...
        if self.coordinates != (None, None) and self.coordinates[0] is not None and self.coordinates[1] is not None:
            try:
    ...
    def get_coordinates(self, location):
        ...
        try:
            headers = {'User-Agent': 'Sleepless Bot'}
    ...
    async def update_embed(self, interaction: discord.Interaction):
        if self.latitude is None or self.longitude is None:
            ...
            return
        embed = discord.Embed(title=f" Map of {self.location}", color=0x006fb9)
        ...
        embed.set_footer(text="Made by Sleepless Development™")
        try:
            if interaction.message is not None:
                await interaction.message.edit(embed=embed, view=self)
            else:
                await interaction.response.send_message("No message to edit.", ephemeral=True)
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author:
            await interaction.response.send_message(
                embed=discord.Embed(description="Sorry only the requested author can control this", color=0x006fb9),
                ephemeral=True
            )
            return False
        return True
    ...
    @discord.ui.button(label="", emoji="<:feast_delete:1400140670659989524>", style=ButtonStyle.danger)
    async def delete_embed(self, interaction: discord.Interaction, button: ui.Button):
        try:
            if interaction.message is not None:
                await interaction.message.delete()
            else:
                await interaction.response.send_message("No message to delete.", ephemeral=True)
        except Exception as e:
    ...
    async def enter_address(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            if self.coordinates == (None, None) or self.coordinates[0] is None or self.coordinates[1] is None:
                await interaction.response.send_message("Failed to retrieve coordinates for the address. Please try again.", ephemeral=True)
            else:
                try:
                    self.latitude, self.longitude = float(self.coordinates[0]), float(self.coordinates[1])
                except (TypeError, ValueError):
                    await interaction.response.send_message("Failed to convert coordinates to float.", ephemeral=True)
                    return
                self.update_map()
                await self.update_embed(interaction)
...
class Map(commands.Cog):
    ...
    async def map(self, ctx, *, location: str):
        ...
        if view.coordinates == (None, None):
            ...
            return
        embed = discord.Embed(title=f" Map of {location}", color=0x006fb9)
